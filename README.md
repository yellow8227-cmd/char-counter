...
색은 전부 CSS 변수로 잡혀 있어 두 벌이 자동으로 나옵니다. 아무것도 안 고르면
**기기 설정을 따라가고**, 머리말의 ☾ 단추로 고르면 그 선택을 기억합니다.

브랜드 띠(«지키는 세 가지»)만 뒤집힙니다 — 밝은 화면에서는 검은 바탕에 노란 마크,
어두운 화면에서는 노란 바탕에 검은 마크. 페이지에서 가장 센 자리는 한 번만 씁니다.

> 색을 새로 넣을 때는 **반드시 변수로** 넣으세요. 값을 직접 적으면 한쪽 테마에서
> 글자가 사라집니다. 실제로 그렇게 두 번 깨졌습니다.

## 없는 주소로 들어왔을 때

`404.html` 이 받습니다. 홈의 `<style>` 을 그대로 복사해 쓰므로,
**홈의 색이나 글꼴을 바꾸면 이 파일도 같이 바꿔야 합니다.**
