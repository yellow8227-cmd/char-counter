...
## 6. 💌 피드백 표 (게임 안 "만든 사람에게" 창)

설정 → **💌 만든 사람에게 한마디** 로 들어오는 글이 여기 쌓입니다.
표가 없으면 게임이 글을 **복사해 줘서** 사람이 다시 쓰는 일은 없지만, 표가 있어야 자동으로 모입니다.
...
create table if not exists feedback (
  ...
  text       text not null,
  name       text,          -- 닉네임 (없으면 빈 값)
  ver        text,
  env        text           -- 화면 크기 · 픽셀비 · 언어 · 브라우저
);
...
-- select 정책을 만들지 않는다 = 대시보드(서비스 키)로만 읽을 수 있다
```

### 읽는 법

Supabase 대시보드 → **Table Editor → feedback** 에서 그냥 보시면 됩니다.
많아지면 이렇게 추려 보세요.

```sql
select created_at, kind, name, text, env
  from feedback
 where created_at > now() - interval '7 days'
 order by created_at desc;

-- 무엇이 제일 많이 오나
select kind, count(*) from feedback group by kind order by 2 desc;
```

> **`select` 정책을 일부러 안 만들었습니다.** 남의 피드백을 아무나 읽을 수 있으면
> 사람들이 솔직하게 안 씁니다. 대시보드에서만 보입니다.
